"""
Day-Trading Decision Dashboard
==============================
Decision support, risk management, a simulated (paper) trade journal, and an
OPTIONAL broker connection (Alpaca) that defaults to PAPER / SANDBOX only.

Safety posture:
- Default mode is PAPER ONLY (offline journal or Alpaca paper). No live orders.
- Live trading is hidden unless explicitly enabled in Streamlit secrets, and
  even then requires an on-screen warning plus a second confirmation per order.
- API keys live in Streamlit secrets and are never printed or logged.
- The app gives NO buy/sell recommendations and does NOT guarantee profits.
...
# Optional broker SDK (Alpaca). The app runs fine without it — the broker tab
# just shows an install hint. Importing is wrapped so a missing package or any
# import error can never crash the dashboard.
try:
...
def render_price_with_levels(data: pd.DataFrame, levels: dict, tail: int = 120):
    """Price + VWAP/EMAs with horizontal support/resistance lines (matplotlib)."""
    ...
    if HAS_MPL:
        ...
        ax.plot(x, d["Close"], color="#111", lw=1.4, label="Price")
        if "VWAP" in d:
            ax.plot(x, d["VWAP"], color="#8e24aa", lw=1.0, ls="--", label="VWAP")
        if "EMA9" in d:
            ax.plot(x, d["EMA9"], color="#1e88e5", lw=0.8, label="EMA9")
        if "EMA20" in d:
            ax.plot(x, d["EMA20"], color="#fb8c00", lw=0.8, label="EMA20")
        lvl_styles = {
            "prev_high": ("Prev High", "#c62828"), "prev_low": ("Prev Low", "#2e7d32"),
            "pm_high": ("PM High", "#ef6c00"), "pm_low": ("PM Low", "#00897b"),
            "day_high": ("Day High", "#b71c1c"), "day_low": ("Day Low", "#1b5e20"),
        }
        for key, (lbl, col) in lvl_styles.items():
            val = levels.get(key)
            if val is not None:
                ax.axhline(val, color=col, lw=0.8, ls=":", alpha=0.8)
                ax.text(len(d) - 1, val, f" {lbl}", color=col, fontsize=7,
                        va="center", ha="left")
        ax.set_title("Price with VWAP, EMAs & key levels", fontsize=11)
        ax.set_xlabel("Recent bars", fontsize=9)
        ax.legend(fontsize=7, loc="upper left")
        fig.tight_layout()
        st.pyplot(fig)
        plt.close(fig)
    else:
        st.line_chart(d[["Close", "VWAP", "EMA9", "EMA20"]])
...
WATCHLIST_DEFAULT = ["SPY", "QQQ", "NVDA", "AAPL", "MSFT", "AMD",
                     "TSLA", "META", "AMZN"]


@st.cache_data(ttl=300, show_spinner=False)
def fetch_levels(ticker: str):
    """Daily bars (for prior-day H/L) and intraday-with-premarket bars."""
    daily = pd.DataFrame()
    ...
    try:
        t = yf.Ticker(ticker)
        daily = t.history(period="6d", interval="1d", prepost=False, actions=False)
    except Exception:
        daily = pd.DataFrame()
    try:
        intraday = yf.Ticker(ticker).history(period="1d", interval="5m",
                                             prepost=True, actions=False)
        if intraday is not None and not intraday.empty and intraday.index.tz is not None:
            try:
...
def scan_ticker(ticker: str, interval: str, period: str,
                atr_low: float, atr_high: float, relvol_low: float):
    """Compute one scanner row for a ticker (cached 60s for auto-refresh)."""
    try:
    ...
    d = add_indicators(df)
    last = d.iloc[-1]
    cfg = Config(relvol_low=relvol_low, atr_low=atr_low, atr_high=atr_high)
    flags = detect_flags(d.index[-1], last, cfg)
    label, bias, _ = score_bar(last, flags, cfg)
    qs, _ = quality_score(last, cfg)
    strength = bias_strength(last, cfg)
    trend = ("↑ Up" if bias == "long" else "↓ Down" if bias == "short"
             else "→ Flat")
    return {
        "Ticker": ticker,
        ...
        "Bias": bias,
        "Strength": strength,
        "RelVol": round(float(last["RelVol"]), 2) if pd.notna(last["RelVol"]) else None,
        "ATR%": round(float(last["ATR_pct"]) * 100, 2) if pd.notna(last["ATR_pct"]) else None,
        "Trend": trend,
    }


# --------------------------------------------------------------------------- #
# Broker integration (Alpaca) — PAPER/SANDBOX by default. Keys come from
# Streamlit secrets and are NEVER printed or returned in any message.
# --------------------------------------------------------------------------- #
BROKER_ORDERS_PATH = Path("broker_orders.csv")


def _secret_section(name: str):
    """Safely read a section from st.secrets without ever raising/leaking."""
    try:
        sec = st.secrets[name]
        return sec
    except Exception:
...
def broker_live_allowed() -> bool:
    """Live trading is only offered if secrets explicitly opt in."""
    b = _secret_section("broker")
    if b is None:
        return False
    try:
        return bool(b.get("allow_live", False))
    except Exception:
        return False


def _broker_keys(mode: str):
    """Return (key, secret, error). mode in {'paper','live'}. Never logs keys."""
    b = _secret_section("broker")
    if b is None:
        return None, None, "No [broker] section found in Streamlit secrets."
    try:
        sect = b.get(mode, None)
    except Exception:
        sect = None
    if sect is None:
        return None, None, f"No [broker.{mode}] keys found in secrets."
    try:
        key = sect.get("api_key")
        secret = sect.get("secret_key")
    except Exception:
        key = secret = None
    if not key or not secret:
        return None, None, f"Missing api_key/secret_key for {mode} in secrets."
    return key, secret, None


def broker_secrets_status(mode: str) -> str:
    """Human-readable status WITHOUT exposing any key material."""
    if not ALPACA_OK:
        return "SDK missing"
    _, _, err = _broker_keys(mode)
    return "configured" if err is None else "not configured"


def make_client(mode: str):
    """Create an Alpaca TradingClient. Returns (client, error). paper=True for
    sandbox. Errors are sanitized to a type name so keys can't leak."""
    if not ALPACA_OK:
        return None, ("alpaca-py is not installed. Add 'alpaca-py' to "
                      "requirements.txt and redeploy.")
    key, secret, err = _broker_keys(mode)
    if err:
        return None, err
    try:
        client = TradingClient(key, secret, paper=(mode == "paper"))
        return client, None
    except Exception as e:  # pragma: no cover - depends on SDK/network
        return None, f"Could not initialize broker client ({type(e).__name__})."


def broker_account(client):
    try:
        ...
        return None, f"Account fetch failed ({type(e).__name__})."


def broker_positions(client):
    try:
        rows = []
        ...
        return None, f"Positions fetch failed ({type(e).__name__})."


def broker_open_orders(client):
    try:
        ...
        return None, f"Open-orders fetch failed ({type(e).__name__})."


def broker_submit(client, symbol, side, otype, qty,
                  limit_price=None, stop_price=None,
                  take_profit=None, stop_loss=None):
    """Submit an order. Returns (result, error). Bracket used only when BOTH a
    take-profit and stop-loss are supplied (on market/limit orders)."""
    try:
        oside = OrderSide.BUY if side == "buy" else OrderSide.SELL
        tif = TimeInForce.DAY
        use_bracket = (take_profit is not None) and (stop_loss is not None) \
            and otype in ("market", "limit")
        tp = TakeProfitRequest(limit_price=float(take_profit)) if use_bracket else None
        sl = StopLossRequest(stop_price=float(stop_loss)) if use_bracket else None
        cls = OrderClass.BRACKET if use_bracket else None
        if otype == "market":
            req = MarketOrderRequest(symbol=symbol, qty=qty, side=oside,
                                     time_in_force=tif, order_class=cls,
                                     take_profit=tp, stop_loss=sl)
        elif otype == "limit":
            req = LimitOrderRequest(symbol=symbol, qty=qty, side=oside,
                                    time_in_force=tif, limit_price=float(limit_price),
                                    order_class=cls, take_profit=tp, stop_loss=sl)
        else:  # stop
            req = StopOrderRequest(symbol=symbol, qty=qty, side=oside,
                                   time_in_force=tif, stop_price=float(stop_price))
        o = client.submit_order(order_data=req)
        return {"id": str(o.id), "status": str(o.status), "symbol": symbol,
                "side": side, "type": otype, "qty": qty}, None
    except Exception as e:
        return None, f"Order rejected/failed ({type(e).__name__})."


def broker_cancel_all(client):
    try:
        client.cancel_orders()
        return True, None
    except Exception as e:
        return None, f"Cancel-all failed ({type(e).__name__})."


def broker_flatten(client):
    try:
        client.close_all_positions(cancel_orders=True)
        return True, None
    except Exception as e:
        return None, f"Flatten failed ({type(e).__name__})."


def order_risk_check(qty, ref_price, stop_price, account_value, max_risk_pct):
    """Return None if risk can't be computed (no stop), else a dict with the
    dollar risk, the allowed cap, and whether it's within limits."""
    if not stop_price or stop_price <= 0 or not ref_price or ref_price <= 0:
        return None
    risk_ps = abs(float(ref_price) - float(stop_price))
    risk_dollars = float(qty) * risk_ps
    allowed = float(account_value) * float(max_risk_pct) / 100.0
    return {"risk_dollars": risk_dollars, "allowed": allowed,
            "ok": risk_dollars <= allowed + 1e-9}


def load_broker_orders() -> list:
    if not BROKER_ORDERS_PATH.exists():
        return []
    try:
        df = pd.read_csv(BROKER_ORDERS_PATH)
        return [] if df.empty else df.to_dict("records")
    except Exception:
        return []


def save_broker_orders(orders: list):
    try:
        if orders:
            pd.DataFrame(orders).to_csv(BROKER_ORDERS_PATH, index=False)
        elif BROKER_ORDERS_PATH.exists():
            BROKER_ORDERS_PATH.unlink()
    except Exception:
        pass
...
def trend_bias(row: pd.Series) -> str:
    """Relaxed bias: price vs VWAP + EMA9 vs EMA20. EMA200 is a confidence
    factor (see bias_strength), not a hard requirement."""
    c, v = row["Close"], row["VWAP"]
    e9, e20 = row["EMA9"], row["EMA20"]
    if pd.isna([c, v, e9, e20]).any():
        return "none"
    if c > v and e9 > e20:
        return "long"
    if c < v and e9 < e20:
...
def bias_strength(row: pd.Series, cfg: "Config") -> str:
    """Weak / Moderate / Strong from confirmation factors (EMA200 alignment,
    participation, healthy volatility). '—' when there's no bias."""
    bias = trend_bias(row)
    if bias == "none":
        return "—"
    e20, e200 = row.get("EMA20"), row.get("EMA200")
    rv, a = row.get("RelVol"), row.get("ATR_pct")
    factors = 0
    if pd.notna(e20) and pd.notna(e200):
        if (bias == "long" and e20 > e200) or (bias == "short" and e20 < e200):
            factors += 1
    if pd.notna(rv) and rv >= 1.2:
        factors += 1
    if pd.notna(a) and cfg.atr_low <= a <= cfg.atr_high:
        factors += 1
    return "Strong" if factors >= 3 else "Moderate" if factors == 2 else "Weak"


def quality_score(row: pd.Series, cfg: "Config"):
    """Trade Quality Score in [-5, +5] with a per-factor breakdown."""
    c, v = row.get("Close"), row.get("VWAP")
    e9, e20, e200 = row.get("EMA9"), row.get("EMA20"), row.get("EMA200")
    rv, a = row.get("RelVol"), row.get("ATR_pct")
    s, pts = 0, []
    if pd.notna(c) and pd.notna(v):
        if c > v:
            s += 1; pts.append(("Price above VWAP", +1))
        elif c < v:
            s -= 1; pts.append(("Price below VWAP", -1))
    if pd.notna(e9) and pd.notna(e20):
        if e9 > e20:
            s += 1; pts.append(("EMA9 above EMA20", +1))
        elif e9 < e20:
            s -= 1; pts.append(("EMA9 below EMA20", -1))
    if pd.notna(e20) and pd.notna(e200):
        if e20 > e200:
            s += 1; pts.append(("EMA20 above EMA200", +1))
        elif e20 < e200:
            s -= 1; pts.append(("EMA20 below EMA200", -1))
    if pd.notna(rv):
        if rv > 1.5:
            s += 1; pts.append(("RelVol > 1.5", +1))
        elif rv < cfg.relvol_low:
            s -= 1; pts.append((f"RelVol < {cfg.relvol_low:.2f}", -1))
    if pd.notna(a):
        if cfg.atr_low <= a <= cfg.atr_high:
            s += 1; pts.append(("ATR healthy", +1))
        else:
            s -= 1; pts.append(("ATR unhealthy", -1))
    return max(-5, min(5, s)), pts


def quality_interpretation(s: int) -> str:
    if s >= 4:
...
def why_not_green(row: pd.Series, flags: dict, cfg: "Config") -> list:
    """List the conditions preventing a Green score (empty if already Green)."""
    fails = []
    if flags.get("open_5min"):
        fails.append("First 5 minutes after the open")
    if flags.get("pre_news"):
        fails.append("Inside a news / event window")
    if flags.get("low_liquidity"):
        fails.append("Low liquidity / wide spread")
    c, v = row.get("Close"), row.get("VWAP")
    e9, e20 = row.get("EMA9"), row.get("EMA20")
    if pd.notna(c) and pd.notna(v) and pd.notna(e9) and pd.notna(e20):
        price_up, ema_up = c > v, e9 > e20
        if trend_bias(row) == "none":
            if price_up != ema_up:
                fails.append("Price and EMAs disagree (mixed trend)")
            else:
                fails.append("Trend not clearly aligned")
    rv = row.get("RelVol")
    if pd.notna(rv) and rv < cfg.relvol_low:
        fails.append(f"Low volume (RelVol {rv:.2f}×)")
    a = row.get("ATR_pct")
    if pd.notna(a):
        if a < cfg.atr_low:
            fails.append(f"ATR too low ({a*100:.2f}%)")
        elif a > cfg.atr_high:
            fails.append(f"ATR too high ({a*100:.2f}%)")
    if flags.get("lunch_chop"):
        fails.append("Lunch-chop window")
    return fails


def evaluate_trade(entry, stop, target, account, risk_pct):
    """Risk/reward + sizing for a hypothetical trade (analysis only)."""
    risk_ps = abs(entry - stop)
    side = "long" if entry >= stop else "short"
    reward_ps = (target - entry) if side == "long" else (entry - target)
    rr = (reward_ps / risk_ps) if risk_ps > 0 else 0.0
    max_loss = account * risk_pct / 100.0
    shares = int(max_loss // risk_ps) if risk_ps > 0 else 0
    rating = "green" if rr >= 2 else "yellow" if rr >= 1 else "red"
    return dict(side=side, risk_ps=risk_ps, reward_ps=reward_ps, rr=rr,
                shares=shares, expected_R=rr, rating=rating, max_loss=max_loss)


def checklist_items(row, entry, stop, target, account, risk_pct, cfg):
    """Six-point pre-trade checklist for a hypothetical trade."""
    ev = evaluate_trade(entry, stop, target, account, risk_pct)
    side, bias = ev["side"], trend_bias(row)
    c, v = row.get("Close"), row.get("VWAP")
    rv, a = row.get("RelVol"), row.get("ATR_pct")
    vwap_ok = (pd.notna(c) and pd.notna(v) and
               ((side == "long" and c > v) or (side == "short" and c < v)))
    items = [
        ("Trend alignment", bias != "none" and bias == side),
        ("VWAP alignment", bool(vwap_ok)),
        ("Volume confirmation", bool(pd.notna(rv) and rv >= 1.2)),
        ("Volatility confirmation",
         bool(pd.notna(a) and cfg.atr_low <= a <= cfg.atr_high)),
        ("Risk within limits", bool(ev["risk_ps"] > 0 and ev["shares"] >= 1)),
        ("Reward ≥ 2R", bool(ev["rr"] >= 2)),
    ]
    return items, sum(1 for _, ok in items if ok), ev


def session_return(df: pd.DataFrame):
    """Percent change across the most recent session in df (open -> last close)."""
    if df is None or df.empty:
        return None
    d = df
    if d.index.tz is not None or hasattr(d.index, "date"):
        try:
            last_day = d.index.date[-1]
            d = d[d.index.date == last_day]
        except Exception:
            pass
    if d.empty:
        return None
...
def relative_strength(stock_df, spy_df):
    sr, mr = session_return(stock_df), session_return(spy_df)
    if sr is None or mr is None:
        return None
    diff = sr - mr
    return dict(stock_ret=sr, spy_ret=mr, diff=diff,
                label="Stronger than SPY" if diff >= 0 else "Weaker than SPY")


def compute_levels(daily_df, regular_df, premarket_df, today_date):
    """Support/resistance: prior-day H/L, premarket H/L, current-day H/L."""
    out = {"prev_high": None, "prev_low": None, "pm_high": None,
           "pm_low": None, "day_high": None, "day_low": None}
    try:
        if daily_df is not None and not daily_df.empty:
            prior = daily_df[daily_df.index.date < today_date]
            if not prior.empty:
                out["prev_high"] = float(prior["High"].iloc[-1])
                out["prev_low"] = float(prior["Low"].iloc[-1])
    except Exception:
        pass
    try:
        if premarket_df is not None and not premarket_df.empty:
            out["pm_high"] = float(premarket_df["High"].max())
            out["pm_low"] = float(premarket_df["Low"].min())
    except Exception:
        pass
    try:
        if regular_df is not None and not regular_df.empty:
            out["day_high"] = float(regular_df["High"].max())
            out["day_low"] = float(regular_df["Low"].min())
    except Exception:
        pass
    return out
...
def score_bar(row: pd.Series, flags: dict, cfg: Config):
    ...
    if bias in ("long", "short"):
        ...
        reasons.append(f"Trend aligned ({'bullish' if bias=='long' else 'bearish'}): "
                       "price vs VWAP and EMA9/EMA20 agree")
    else:
        pts -= 1
...
# Mobile-friendly tweaks: let the tab bar wrap instead of cramming/scrolling,
# give buttons/inputs touch-friendly height, and tighten side padding on phones.
st.markdown(
    """
    <style>
      .stTabs [data-baseweb="tab-list"] { flex-wrap: wrap; gap: 0.25rem; }
      .stTabs [data-baseweb="tab"] { padding: 0.4rem 0.7rem; }
...
if "broker_orders" not in ss:
    ss.broker_orders = load_broker_orders()
ss.setdefault("pending_order", None)

with st.sidebar:
    # ---- Mode / Broker Settings ----------------------------------------- #
    st.header("⚙️ Broker Settings")
    _live_ok = broker_live_allowed()
    mode_options = ["Offline paper journal", "Broker paper trading"]
    if _live_ok:
        mode_options.append("Live trading")
    app_mode = st.selectbox("Mode", mode_options, index=0, key="app_mode",
                            help="Default is offline paper. 'Broker paper "
                                 "trading' uses Alpaca's sandbox. Live trading "
                                 "only appears if enabled in secrets.")
    broker_mode = ("paper" if app_mode == "Broker paper trading"
                   else "live" if app_mode == "Live trading" else None)

    if app_mode == "Offline paper journal":
        st.caption("🧪 Offline paper journal · no brokerage · no live orders.")
    else:
        st.caption(f"Alpaca SDK: {'✅ installed' if ALPACA_OK else '❌ missing'} · "
                   f"keys: {broker_secrets_status(broker_mode)}")
        if broker_mode == "live":
            st.error("🔴 LIVE mode selected — real money. Orders still need an "
                     "extra confirmation each time.")
    if not _live_ok:
        st.caption("Live trading is disabled. Set [broker] allow_live = true in "
                   "secrets to unlock it (then it still requires confirmation).")
    st.divider()
...
_mode_tag = {"Offline paper journal": "🧪 Offline paper journal",
             "Broker paper trading": "🟢 Alpaca PAPER (sandbox)",
             "Live trading": "🔴 Alpaca LIVE (real money)"}.get(app_mode, app_mode)
st.caption(f"Mode: **{_mode_tag}** · the app places no orders on its own and "
           "gives no buy/sell advice. Data may be ~15 min delayed. "
           "Educational, not financial advice.")
if app_mode == "Live trading":
    st.error("🔴 **LIVE TRADING IS ON.** Live trading can lose real money. This "
             "app is educational and does not guarantee profits. Every order "
             "still requires an extra confirmation.")
...
# Clear stale sizing / widget state when the ticker changes, so old values
# (e.g. an entry typed for a different symbol like 100/740) never stay stuck.
_SIZING_KEYS = ["risk_entry", "risk_stop", "risk_target",
                "pf_entry", "pf_stop", "pf_target", "pf_shares"]
if ss.get("_last_ticker") != ticker:
    ss["_last_ticker"] = ticker
    ss.pop("sizing", None)
    ss.pending_trade = None
    ss.trade_stage = "entry"
    for _k in _SIZING_KEYS:
        ss.pop(_k, None)

tabs = st.tabs(["🟢 Decision", "🛡️ Risk & Sizing", "🧪 Paper Trade & Journal",
                "🔁 Backtest", "📈 Strategy Review", "🎯 Estimator",
                "🧭 AI Coach", "📡 Market Scanner", "🔌 Broker"])
...
with tabs[0]:
    ...
    with st.container(border=True):
        ...
        strength = bias_strength(latest, cfg)
        head[2].metric("Bias", bmap[bias],
                       delta=None if strength == "—" else strength)
        ...
        # Trade Quality Score (-5..+5)
        qs, qpts = quality_score(latest, cfg)
        qcol = st.columns([1, 2])
        qcol[0].metric("Trade Quality Score", f"{qs:+d}",
                       delta=quality_interpretation(qs))
        with qcol[1]:
            st.caption("Score range −5 (strong short) to +5 (strong long).")
            if qpts and not clean:
                st.write(" · ".join(f"{lbl} ({p:+d})" for lbl, p in qpts))
        ...
        # Why Not Green
        if label != "Green":
            fails = why_not_green(latest, flags, cfg)
            if fails:
                st.markdown("**Why not green?**")
                for f in fails:
                    st.write(f"- ❌ {f}")
    ...
    # Relative strength vs SPY (skip when the ticker IS SPY)
    if ticker != "SPY":
        try:
            spy_df = fetch("SPY", period, interval)
            rs = relative_strength(data, spy_df)
        except Exception:
            rs = None
        if rs:
            rcol = st.columns(3)
            rcol[0].metric(f"{ticker} session", f"{rs['stock_ret']:+.2f}%")
            rcol[1].metric("SPY session", f"{rs['spy_ret']:+.2f}%")
            rcol[2].metric("Relative strength", f"{rs['diff']:+.2f}%",
                           delta=rs["label"])
            if rs["diff"] >= 0:
                st.success(f"📈 Relative strength: {rs['label']}")
            else:
                st.warning(f"📉 Relative strength: {rs['label']}")

    # Support / resistance levels
    try:
        daily_df, intraday_full = fetch_levels(ticker)
        today_date = latest_ts.date()
        if intraday_full is not None and not intraday_full.empty and \
                intraday_full.index.tz is not None:
            t = intraday_full.index.time
            import datetime as _dt
            pm_mask = [x < _dt.time(9, 30) for x in t]
            rg_mask = [_dt.time(9, 30) <= x < _dt.time(16, 0) for x in t]
            today_mask = intraday_full.index.date == today_date
            pm_df = intraday_full[[a and b for a, b in zip(pm_mask, today_mask)]]
            rg_df = intraday_full[[a and b for a, b in zip(rg_mask, today_mask)]]
        else:
            pm_df = rg_df = pd.DataFrame()
        levels = compute_levels(daily_df, rg_df, pm_df, today_date)
    except Exception:
        levels = {}
    ...
    if any(v is not None for v in levels.values()):
        ...
        lv_rows = [
            ("Previous day high", levels.get("prev_high")),
            ("Previous day low", levels.get("prev_low")),
            ("Premarket high", levels.get("pm_high")),
            ("Premarket low", levels.get("pm_low")),
            ("Current day high", levels.get("day_high")),
            ("Current day low", levels.get("day_low")),
        ]
        lv_df = pd.DataFrame(
            [{"Level": n, "Price": (f"{v:.2f}" if v is not None else "—"),
              "vs now": ("—" if v is None else
                         ("above" if v > price else "below"))}
             for n, v in lv_rows])
        st.dataframe(lv_df, use_container_width=True, hide_index=True)

    if not clean:
        with st.expander("ℹ️ Indicator explainers (beginner)"):
            explain("vwap"); explain("ema200"); explain("stack")
            explain("atr"); explain("relvol")
        st.divider()
        render_price_with_levels(data, levels if isinstance(levels, dict) else {})
        st.markdown("**Volume**")
...
with tabs[1]:
    ...
    target_default = safe_price(price + 2 * atr_offset, fallback=price)

    if st.button("🔄 Reset to live price"):
        for _k in ("risk_entry", "risk_stop", "risk_target"):
            ss.pop(_k, None)
        ss.pop("sizing", None)
        st.rerun()

    entry = c[0].number_input("Entry price", 0.0, 1e6, entry_default,
                              step=0.01, key="risk_entry")
    stop = c[1].number_input("Stop price", 0.0, 1e6, stop_default,
                             step=0.01, key="risk_stop")
    target = c[2].number_input("Target price", 0.0, 1e6, target_default,
                               step=0.01, key="risk_target")

    # --- Trade evaluation -------------------------------------------------
    ev = evaluate_trade(entry, stop, target, account, risk_pct)
    st.markdown("#### Trade evaluation")
    e1 = st.columns(3)
    e1[0].metric("Risk / share", f"${ev['risk_ps']:.2f}")
    e1[1].metric("Reward / share", f"${ev['reward_ps']:.2f}")
    e1[2].metric("Risk : reward", f"{ev['rr']:.2f}R")
    e2 = st.columns(3)
    e2[0].metric("Suggested shares", f"{ev['shares']:,}")
    e2[1].metric("Max loss / trade", f"${ev['max_loss']:,.2f}")
    e2[2].metric("Expected R (if target hit)", f"{ev['expected_R']:+.2f}R")
    if ev["risk_ps"] <= 0:
        st.info("Set a stop different from entry to evaluate the trade.")
    elif ev["rating"] == "green":
        st.success(f"🟢 Worth considering — reward:risk {ev['rr']:.2f} ≥ 2 "
                   f"({ev['side']} setup). You still make the final call.")
    elif ev["rating"] == "yellow":
        st.warning(f"🟡 Marginal — reward:risk {ev['rr']:.2f} is between 1 and 2.")
    else:
        st.error(f"🔴 Skip — reward:risk {ev['rr']:.2f} is below 1.")

    # --- Checklist score --------------------------------------------------
    st.markdown("#### Checklist score")
    items, cscore, _ = checklist_items(latest, entry, stop, target,
                                       account, risk_pct, cfg)
    cc = st.columns(2)
    for i, (lbl, ok) in enumerate(items):
        cc[i % 2].write(f"{'✅' if ok else '❌'} {lbl}")
    if cscore >= 5:
        st.success(f"Checklist score: **{cscore}/6**")
    elif cscore >= 3:
        st.warning(f"Checklist score: **{cscore}/6**")
    else:
        st.error(f"Checklist score: **{cscore}/6**")

    if ev["side"] == "short" and target <= 0:
        st.warning("This short's target prices below zero — unrealistic setup "
                   "(entry far below stop). Check your entry and stop.")

    # Store clamped values for the Paper Trade form (never negative).
    ss["sizing"] = dict(side=ev["side"],
                        entry=safe_price(entry, price),
                        stop=safe_price(stop, price),
                        target=safe_price(target, price),
                        shares=max(0, int(ev["shares"])),
                        rr=ev["rr"] if ev["rr"] > 0 else 2.0)
    ...
    if ss.trade_stage == "entry" or ss.pending_trade is None:
        with st.form("open_paper", clear_on_submit=False):
            ...
            p_entry = cc[1].number_input("Entry", 0.0, 1e6,
                                         safe_price(sizing["entry"], price),
                                         step=0.01, key="pf_entry")
            p_stop = cc[2].number_input("Stop", 0.0, 1e6,
                                        safe_price(sizing["stop"], price),
                                        step=0.01, key="pf_stop")
            p_target = cc[3].number_input("Target", 0.0, 1e6,
                                          safe_price(sizing["target"], price),
                                          step=0.01, key="pf_target")
...
with tabs[7]:
    ...
    st.caption("Scan several tickers at once using the same scoring as the "
               "Decision tab. Analysis only — nothing here places an order.")

    wl_raw = st.text_input("Watchlist (comma-separated)",
                           value=", ".join(WATCHLIST_DEFAULT),
                           key="scanner_watchlist")
    watch = [t.strip().upper() for t in wl_raw.split(",") if t.strip()]
    watch = list(dict.fromkeys(watch))[:25]

    sc = st.columns(3)
    sort_by = sc[0].selectbox("Sort by", ["Quality", "Score", "RelVol",
                                          "ATR%", "Price", "Ticker"], index=0)
    descending = sc[1].toggle("Descending", value=True)
    auto = sc[2].toggle("Auto-refresh 60s", value=False)
    ...
    def _build_scanner():
        ...
        for i, tk in enumerate(watch):
            try:
                r = scan_ticker(tk, interval, period, atr_low, atr_high, relvol_low)
            except Exception:
                r = None
            if r:
                rows.append(r)
        ...
        prog.empty()
        if not rows:
            st.warning("No data returned — check the tickers/interval, or try "
                       "again (free data is delayed and can be rate-limited).")
            return
        sdf = pd.DataFrame(rows)
        rank = {"Green": 0, "Yellow": 1, "Red": 2}
        if sort_by == "Score":
            sdf = (sdf.assign(_r=sdf["Score"].map(rank).fillna(3))
                      .sort_values("_r", ascending=not descending)
                      .drop(columns="_r"))
        else:
            sdf = sdf.sort_values(sort_by, ascending=not descending,
                                  na_position="last")
        sdf = sdf.reset_index(drop=True)

        def _row_color(row):
            col = {"Green": "#1b5e2055", "Yellow": "#f9a82555",
                   "Red": "#c6282855"}.get(row["Score"], "")
            return [f"background-color: {col}"] * len(row)

        try:
            st.dataframe(sdf.style.apply(_row_color, axis=1),
                         use_container_width=True, hide_index=True)
        except Exception:
            st.dataframe(sdf, use_container_width=True, hide_index=True)
        greens = int((sdf["Score"] == "Green").sum())
        st.caption(f"{len(sdf)} tickers · {greens} green. Tap a column header "
                   "to re-sort. Data may be ~15 min delayed. Not a "
                   "recommendation to buy or sell anything.")

    if auto and hasattr(st, "fragment"):
        try:
            st.fragment(run_every=60)(_build_scanner)()
        except Exception:
            _build_scanner()
    else:
...
with tabs[8]:
    st.subheader("🔌 Broker")
    st.caption("Connect to Alpaca (paper/sandbox by default) to see real account "
               "info and place orders you confirm yourself. This tool places no "
               "orders on its own and gives no buy/sell advice.")

    if app_mode == "Offline paper journal":
        st.info("You're in **Offline paper journal** mode — no broker connection. "
                "Switch to **Broker paper trading** in the sidebar (⚙️ Broker "
                "Settings) to connect Alpaca's sandbox.")
    elif not ALPACA_OK:
        st.error("The Alpaca SDK isn't installed. Add `alpaca-py` to "
                 "requirements.txt and redeploy, then add your keys to secrets.")
        with st.expander("How to set up secrets"):
            st.code(
                ...
                "api_key = \"PK...\"\n"
                "secret_key = \"...\"\n\n"
                "# Only if you ever enable live (not recommended):\n"
                "# [broker.live]\n"
                "# api_key = \"AK...\"\n"
                "# secret_key = \"...\"\n", language="toml")
    else:
        client, cerr = make_client(broker_mode)
        if cerr:
            st.error(f"Not connected: {cerr}")
            with st.expander("How to set up secrets"):
                st.code(
                    "# .streamlit/secrets.toml\n"
                    "[broker]\n"
            ...
            acct, aerr = broker_account(client)
            if aerr:
                st.error(f"🔴 Connection problem: {aerr}")
                acct = None
            else:
                tag = "🟢 PAPER" if broker_mode == "paper" else "🔴 LIVE"
                st.success(f"{tag} connected — account status "
                           f"{acct.get('status', '—')}.")
                m = st.columns(4)
                m[0].metric("Equity", f"${acct['equity']:,.2f}")
                m[1].metric("Buying power", f"${acct['buying_power']:,.2f}")
                m[2].metric("Cash", f"${acct['cash']:,.2f}")
                m[3].metric("Portfolio value", f"${acct['portfolio_value']:,.2f}")

            # Positions & open orders
            pcol, ocol = st.columns(2)
            with pcol:
                st.markdown("**Current positions**")
                pos, perr = broker_positions(client)
                if perr:
                    st.warning(perr)
                elif pos:
            ...
            with ocol:
                ...
                orders, oerr = broker_open_orders(client)
                if oerr:
                    st.warning(oerr)
                elif orders:
            ...
            ec = st.columns(2)
            if ec[0].button("🧹 Cancel all open orders"):
                ok, e = broker_cancel_all(client)
                st.success("Cancel-all sent.") if ok else st.error(e)
            if ec[1].button("📉 Flatten positions (close all)"):
                ok, e = broker_flatten(client)
                st.success("Flatten sent (positions closing).") if ok else st.error(e)

            st.divider()

            # ---- Order ticket ------------------------------------------------
            st.markdown("#### Place an order")
            acct_value = acct["equity"] if acct else account
            with st.form("broker_order"):
                oc = st.columns(4)
                o_symbol = oc[0].text_input("Symbol", value=ticker).strip().upper()
                o_side = oc[1].selectbox("Side", ["buy", "sell"])
                o_type = oc[2].selectbox("Order type", ["market", "limit", "stop"])
                o_qty = oc[3].number_input("Quantity (shares)", 1, 1_000_000, 1)
                pc = st.columns(3)
                o_limit = pc[0].number_input("Limit price (limit orders)", 0.0,
                                             1e6, safe_price(price, 1.0), step=0.01)
                o_stop = pc[1].number_input("Stop price (stop / risk calc)", 0.0,
                                            1e6, 0.0, step=0.01)
                o_tp = pc[2].number_input("Take-profit (bracket)", 0.0, 1e6, 0.0,
                                          step=0.01)
                review = st.form_submit_button("Review order →", type="primary")
                if review:
                    ss.pending_order = dict(
                        symbol=o_symbol, side=o_side, otype=o_type, qty=int(o_qty),
                        limit=float(o_limit), stop=float(o_stop), tp=float(o_tp))

            pend = ss.get("pending_order")
            if pend:
                st.markdown("#### Order preview")
                ref = pend["limit"] if pend["otype"] == "limit" else price
                stop_for_risk = pend["stop"] if pend["stop"] > 0 else None
                rc = order_risk_check(pend["qty"], ref, stop_for_risk,
                                      acct_value, risk_pct)
                tp_val = pend["tp"] if pend["tp"] > 0 else None
                sl_val = pend["stop"] if pend["stop"] > 0 else None

                st.write(f"**{pend['side'].upper()} {pend['qty']} "
                         f"{pend['symbol']}** · {pend['otype']} order"
                         + (f" @ limit {pend['limit']:.2f}" if pend['otype'] == 'limit' else "")
                         + (f" · stop {pend['stop']:.2f}" if pend['otype'] == 'stop' else ""))
                if tp_val and sl_val:
                    st.caption(f"Bracket: take-profit {tp_val:.2f} · "
                               f"stop-loss {sl_val:.2f}")
                est_cost = pend["qty"] * ref
                pv = st.columns(2)
                pv[0].metric("Est. notional", f"${est_cost:,.2f}")
                if rc is not None:
                    pv[1].metric("Est. risk vs cap",
                                 f"${rc['risk_dollars']:,.2f} / ${rc['allowed']:,.2f}")

                # --- Risk guardrail (block over-limit) ---
                blocked = False
                if rc is None:
                    st.warning("⚠️ No stop set — risk can't be bounded. Add a stop "
                               "price to size risk, or proceed with caution.")
                elif not rc["ok"]:
                    blocked = True
                    st.error(f"🚫 Blocked: risk ${rc['risk_dollars']:,.2f} exceeds "
                             f"your {risk_pct:.1f}% cap (${rc['allowed']:,.2f}). "
                             "Reduce quantity or tighten the stop.")

                # --- Red-score guardrail ---
                sym_label = label if pend["symbol"] == ticker else None
                if sym_label is None:
                    srow = scan_ticker(pend["symbol"], interval, period,
                                       atr_low, atr_high, relvol_low)
                    sym_label = srow["Score"] if srow else None
                red_reason = ""
                if sym_label == "Red":
                    st.warning(f"⚠️ {pend['symbol']} currently scores RED.")
                    red_reason = st.text_input("Why override the RED score? "
                                               "(required)", key="broker_red")

                # --- Confirmations ---
                confirm = st.checkbox("I have reviewed this order and want to "
                                      "submit it.", key="broker_confirm")
                live_confirm = True
                if broker_mode == "live":
                    st.error("**Live trading can lose real money. This app is "
                             "educational and does not guarantee profits.**")
                    live_confirm = st.checkbox(
                        "I understand this is a REAL order with REAL money.",
                        key="broker_live_confirm")

                need_reason = (sym_label == "Red" and not red_reason.strip())
                can_submit = (confirm and live_confirm and not blocked
                              and not need_reason)
                if need_reason:
                    st.caption("Enter a reason to override the RED score.")

                bcols = st.columns(2)
                if bcols[0].button("✅ Submit order", type="primary",
                                   disabled=not can_submit):
                    res, err = broker_submit(
                        client, pend["symbol"], pend["side"], pend["otype"],
                        pend["qty"], limit_price=pend["limit"],
                        stop_price=pend["stop"], take_profit=tp_val,
                        stop_loss=sl_val)
                    rec = {
                        "time": dt.datetime.now().isoformat(timespec="seconds"),
                        "mode": broker_mode, "symbol": pend["symbol"],
                        "side": pend["side"], "type": pend["otype"],
                        "qty": pend["qty"],
                        "limit": pend["limit"] if pend["otype"] == "limit" else "",
                        "stop": pend["stop"] if pend["stop"] > 0 else "",
                        "take_profit": tp_val or "",
                        "score_at_submit": sym_label or "",
                        "override_reason": red_reason.strip(),
                        "status": (res["status"] if res else "REJECTED"),
                        "order_id": (res["id"] if res else ""),
                        "error": err or "",
                    }
                    ss.broker_orders.append(rec)
                    save_broker_orders(ss.broker_orders)
                    ss.pending_order = None
                    if err:
                        st.error(f"Order failed: {err}")
                    else:
                        st.success(f"Order submitted ({broker_mode}): "
                                   f"{res['status']} · id {res['id'][:8]}…")
                        st.rerun()
                if bcols[1].button("Cancel"):
                    ss.pending_order = None
                    st.rerun()

    # ---- Broker order log (always available) ------------------------------
    st.divider()
    st.markdown("#### Broker order log")
    st.caption(f"Every submitted order is saved to `{BROKER_ORDERS_PATH}`.")
    if ss.broker_orders:
        bdf = pd.DataFrame(ss.broker_orders)
        st.dataframe(bdf, use_container_width=True, hide_index=True)
        st.download_button("⬇️ Export broker orders (CSV)",
                           bdf.to_csv(index=False).encode("utf-8"),
                           file_name="broker_orders.csv", mime="text/csv")
        st.caption("To record a *completed* round-trip into your performance "
                   "journal (Strategy Review / Estimator / Coach), close it and "
                   "log it from the 🧪 Paper Trade tab — that keeps win-rate and "
                   "expectancy math based on closed trades only.")
        if st.button("🗑️ Clear broker order log"):
            ss.broker_orders = []
            save_broker_orders(ss.broker_orders)
            st.rerun()
    else:
        st.caption("No broker orders submitted yet.")

st.divider()
st.caption("⚠️ Default mode is paper / sandbox. The app places no orders on its "
           "own, gives no buy/sell advice, and does not guarantee profits. Live "
           "trading risks real money and is your responsibility. Educational "
