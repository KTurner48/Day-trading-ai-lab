...
for line in [l.strip() for l in news_raw.splitlines() if l.strip()]:
    try:
        ...
        news_windows.append(pd.Timestamp(
            dt.datetime(now.year, now.month, now.day, hh, mm), tz=ET))
    except Exception:
        st.sidebar.warning(f"Couldn't parse: '{line}'")
...
with tabs[1]:
    ...
    explain("sizing")
    c = st.columns(3)
    entry = c[0].number_input("Entry price", 0.0, 1e6, round(price, 2), step=0.01)
    default_stop = max(round(price - float(latest["ATR"]), 2), 0.0)
    stop = c[1].number_input("Stop price", 0.0, 1e6, default_stop, step=0.01)
    ...
    if per_share > 0:
        ...
        ss["sizing"] = dict(side=side, entry=entry, stop=stop, target=target,
                            shares=shares, rr=rr)
    else:
...
with tabs[2]:
    ...
    sizing = ss.get("sizing", dict(
        ...
        entry=round(price, 2),
        stop=round(price - float(latest['ATR']), 2),
        target=round(price + 2 * float(latest['ATR']), 2),
        shares=0, rr=2.0))
    ...
    if ss.trade_stage == "entry" or ss.pending_trade is None:
        with st.form("open_paper", clear_on_submit=False):
            ...
            p_entry = cc[1].number_input("Entry", 0.0, 1e6, float(sizing["entry"]), step=0.01)
            p_stop = cc[2].number_input("Stop", 0.0, 1e6, float(sizing["stop"]), step=0.01)
            p_target = cc[3].number_input("Target", 0.0, 1e6, float(sizing["target"]), step=0.01)
            p_shares = cc[4].number_input("Shares", 0, 10_000_000, int(sizing["shares"]))
            reason = st.text_area(
    ...
    if ss.open_trades:
        ...
        for i, tr in enumerate(list(ss.open_trades)):
            ...
            exit_px = mc[0].number_input("Exit @", 0.0, 1e6, round(price, 2),
